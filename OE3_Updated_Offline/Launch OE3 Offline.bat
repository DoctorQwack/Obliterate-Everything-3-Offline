...
if %ERRORLEVEL% neq 0 (
    echo.
    echo Server has stopped with error %ERRORLEVEL%.
    pause
)
exit /b
#>
...
if ($config.remember_mode -and $launchMode -ne "ask") {
    ...
    if ($rem -and ($rem.Trim().ToLower() -in @("y", "yes"))) {
...
function Start-GameInstance($mode = $null) {
    if ($null -eq $mode) {
...
function Execute-ConsoleCommand($inputStr) {
    ...
    switch ($action) {
        ...
        "launch" {
            $reqMode = $null
            if ($parts.Count -gt 1) {
                $reqMode = $parts[1].ToLower()
                if ($reqMode -notin @("flashplayer", "ruffle", "browser", "auto")) {
                    Write-Host "Invalid launch mode: '$reqMode'. Using default." -ForegroundColor Yellow
                    $reqMode = $null
                }
            }
            $proc = Start-GameInstance $reqMode
            if ($proc) {
                $global:instanceCount++
                $inst = [PSCustomObject]@{
                    id = $global:instanceCount
                    process = $proc
                    mode = if ($reqMode) { $reqMode } else { $config.launch_mode }
            ...
            if ($parts.Count -lt 2) {
                ...
                if ($newMode -in @("ask", "flashplayer", "ruffle", "browser", "auto", "converter")) {
                    ...
                    if ($newMode -eq "ask") {
                        ...
                        $script:config.remember_mode = $true
                    }
                    Save-Config
                    Write-Host "Launch mode set to '$newMode' (Remembered: $($script:config.remember_mode))." -ForegroundColor Green
                } else {
...
$firstProc = Start-GameInstance $launchMode
if ($firstProc) {
    $global:instanceCount++
    ...
    try {
            ...
            if ($global:gameInstances.Count -eq 0) {
                ...
                for ($sec = 5; $sec -gt 0; $sec--) {
                    ...
                    for ($ms = 0; $ms -lt 1000; $ms += 100) {
                        # Keep serving HTTP requests during shutdown countdown!
                        if ($contextAsync.IsCompleted) {
                            $c = $l.EndGetContext($contextAsync)
                            $contextAsync = $l.BeginGetContext($null, $null)
        ...
        if ([Console]::KeyAvailable) {
            ...
            if (-not $global:inConsoleMode) {
                ...
                # Capture the key that triggered the console so it's not discarded
